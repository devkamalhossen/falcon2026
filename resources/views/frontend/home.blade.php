...
@section('page_title', 'Epoxy & PU Flooring Services in Bangladesh')
@section('meta_data')
                ...
                <div
                    ...
                    <div class="swiper heroBannerSlider h-full relative">
                        <div class="swiper-wrapper">
                            @foreach ($bannerSlider as $slider)
                                <div class="swiper-slide">
                                    <img loading="lazy" src="{{ asset($slider->image) }}" alt="{{ $slider->alt_name }}"
                        ...
                        <div
                            ...
                            {{--<h1 data-aos="fade-up"
                                class="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl xl:leading-[1.2] font-bold [&_span]:text-secondary [&_br]:max-lg:hidden animate__animated animate__fadeInDown">
                                {{ get_option('site_banner_title') }}
                            </h1>--}}
                            ...
                            <h4 data-aos="fade-up"
                                ...
                                {{ get_option('site_banner_sub_title') }}
                            </h4>

                            @if ($certficateBadges->count() > 0)
                                <div
                                    class="my-6 flex items-center justify-center flex-wrap gap-2.5 animate__animated animate__fadeIn">
                                    @foreach ($certficateBadges as $badge)
                                        <img loading="lazy" class="max-w-sm" src="{{ asset($badge->image) }}"
                ...
                @if ($homeIntro?->video_id)
                    ...
                    @if ($randomId)
                        <div class="relative inline-block max-w-xl w-full aspect-video shrink-0">
                            <a data-fslightbox="gallery" href="https://www.youtube.com/watch?v={{ $randomId }}">
                                <img loading="lazy" class="size-full object-cover"
                                    src="https://img.youtube.com/vi/{{ $randomId }}/hqdefault.jpg"
                                    alt="Falcon Solution Limited" />
    ...
    <section class="relative">
        ...
        <div class="h-[800px] relative">
            <div class="swiper featuredProject h-full">
                <div class="swiper-wrapper">
                    @foreach ($featuredProjects as $slide)
                        <div class="bg-cover swiper-slide text-white relative z-[1] after:absolute after:inset-0 after:bg-black/60 after:z-[-1]"
                            ...
                            <div class="container h-full flex flex-col justify-center">
                                <h4 class="text-xl uppercase tracking-wider">
                                    {{ $slide->status == 1 ? 'Upcoming' : ($slide->status == 2 ? 'Ongoing' : 'Completed') }}
                                </h4>
                                <h2 class="text-4xl lg:text-5xl font-medium uppercase mt-4 mb-8">{{ $slide->name }}</h2>
                                <div class="h-20 flex items-center">
                                    <a href="{{ route('project-details', $slide->slug) }}"
                            ...
                            @foreach ($featuredProjects as $slide)
                                <div class="swiper-slide">
                                    <div class="aspect-[5/3]">
                                        @if ($slide->video)
                                            <video src="{{ asset($slide->video) }}" autoplay muted loop playsinline
    ...
    <section class="relative py-16 lg:py-24 overflow-hidden"
        ...
        <div class="relative container lg:max-w-7xl">
            <h3 class="text-3xl md:text-4xl lg:text-5xl font-bold text-center capitalize mb-12 text-white">
                {{ $achievementData['achievementData']?->title }}
            </h3>
            <p class="mt-4 mb-12 text-2xl text-justify w-full text-white">
                {{ $achievementData['achievementData']?->description }}
            </p>
                ...
                <div class="flex justify-center">
                    <div class="swiper achievementSlider">
                        <div class="swiper-wrapper">
                            ...
                            @foreach ($achievementData['sliders'] as $item)
                                <div class="swiper-slide">
                ...
                <div class="">
                    <div class="flex items-center gap-4 mb-2">
                        <img loading="lazy" class="h-10 w-auto"
                            src="{{ asset('frontend/assets/images/google-logo.png') }}" alt="Falcon Solution Limited">
                        <p class="font-bold text-dark text-xl">Review</p>
                    </div>
                    <div class="flex items-center gap-2">
                        <p class="font-bold text-dark">
                            {{ number_format($googleReviews['avg_rating'], 1) }}
                        </p>
                        <div class="text-amber-500 flex items-center gap-[2px] text-xl">
                            @for ($i = 1; $i <= 5; $i++)
                                @if ($i <= floor($googleReviews['avg_rating']))
                                    {!! config('icon.star') !!}
                                @endif
                            @endfor
                        </div>
                        ...
                        <div class="swiper-slide">
                            <div class="bg-white shadow-lg rounded-2xl p-5 space-y-3 min-h-70">
                                ...
                                <div class="flex items-center space-x-3">
                                    {!! userAvatar($review->image, $review->name) !!}
                                    <div>
        ...
        <div class="relative">
            <div class="container mb-20">
                ...
                <p class="text-[24px] text-center">
                    {{ $latestServices['servicePage']?->section_description }}
                </p>
            </div>
            <div class="swiper exploreSwiper">
                <div class="swiper-wrapper pb-8 md:pb-12 lg:pb-20">
                    @foreach ($latestServices['latestServices'] as $service)
                        <div
                            ...
                            <a href="{{ route('service.detail', $service->slug) }}"
                                ...
                                @elseif(!$service->video && $service->image)
    ...
    <section class="py-12 md:py-20 lg:py-24 bg-white">
        <div class="container">
            <div class="mb-20">
                <h2
                    class="text-3xl text-center md:text-4xl lg:text-5xl xl:text-6xl text-dark font-medium uppercase mb-10 lg:mb-6">
                    {{ $homeCategoryIntro?->title }}
                </h2>
                <p class="text-[24px] text-center">
                    {{ $homeCategoryIntro?->description }}
                </p>
            </div>
            @if ($homeCategoryContent->count() > 0)
                <!-- Vertical Tabs -->
                <div class="md:flex">
                    <!-- Tab List -->
                    <ul id="default-tab" data-tabs-toggle="#default-tab-content" role="tablist"
                        class="flex-column text-sm font-medium text-dark">
                        @foreach ($homeCategoryContent as $key => $content)
                            <!-- Profile Tab -->
                            <li role="presentation">
                                <button id="profile-tab-{{ $key }}"
                                    data-tabs-target="#profile-{{ $key }}" type="button" role="tab"
                                    aria-controls="profile-{{ $key }}"
                    ...
                    <div id="default-tab-content" class="flex-1">
                        @foreach ($homeCategoryContent as $key => $content)
                            <div class="hidden p-6 bg-[#FFFAF4] h-full " id="profile-{{ $key }}"
                                role="tabpanel" aria-labelledby="profile-tab-{{ $key }}">
                                <div class="flex flex-col items-start gap-6">
                                    @if ($content->image)
                                        <img loading="lazy"
                                            ...
                                            src="{{ asset($content->image) }}" alt="{{ $content->name }}">
                                    @endif
                        ...
                        @foreach ($businesses as $business)
                            <div
                                ...
                                <div class="w-[450px] sm:!w-[350px] h-[500px] relative">
                                    <img loading="lazy" src="{{ asset($business?->thumbnail) }}"
                                        alt="{{ $business?->name }}"
                                ...
                                <div
                                    ...
                                    @foreach ($business?->businesses as $busi)
                                        <a class="flex items-center gap-1 text-xl hover:text-primary hover:underline"
                                            href="{{ $busi->link }}">{{ $busi->name }} <span class="text-primary">
                            ...
                            <div class="swiper-slide">
                                ...
                                <div class="group relative overflow-hidden">
                                    <div class="aspect-[519/380] relative">
                                        ...
                                        <img loading="lazy" src="{{ asset($importer->image) }}"
            ...
            <div class="grid gap-8 lg:gap-16 items-center ">
                <div class="space-y-8 py-12 lg:py-20 xl:py-24">
                    ...
                    <div
                        ...
                        <img loading="lazy" src="{{ asset('frontend/assets/images/cta.avif') }}"
                            ...
                            <div class="flex items-center justify-between border-b border-gray-700 pb-4">
                                ...
                                <div class="ml-4">
                                    <span
                                        ...
                                        <a href="{{ route('contact') }}"> {!! config('icon.upRight') !!}</a>
                                    </span>
                                </div>
                            </div>
                        </div>
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-10 md:gap-12 lg:gap-14">
                <div>
                    ...
                    <ul class="flex-col flex gap-1.5 pt-3">
                        ...
                        <a href="{{ route('home') }}" class="flex items-center gap-2 text-[18px]">
                            {!! config('icon.chevronRight') !!}
                            Home
                        </a>
                        <a href="{{ route('our-story') }}" class="flex items-center gap-2 text-[18px]">
                            {!! config('icon.chevronRight') !!}
                            Our Story
                        </a>
                        <a href="{{ route('our-clients') }}" class="flex items-center gap-2 text-[18px]">
                            {!! config('icon.chevronRight') !!}
                            Our Clients
                        </a>
                        <a href="{{ route('services') }}" class="flex items-center gap-2 text-[18px]">
                            {!! config('icon.chevronRight') !!}
                            Services
                        </a>
                        <a href="{{ route('product-list') }}" class="flex items-center gap-2 text-[18px]">
                            {!! config('icon.chevronRight') !!}
                            Product List
                        </a>

                        <a href="{{ route('blogs') }}" class="flex items-center gap-2 text-[18px]">
                            {!! config('icon.chevronRight') !!}
                            Blog
                        </a>
                        @php
                        ...
                        @foreach ($pages as $page)
                            <a href="{{ route('page', $page->slug) }}" class="flex items-center gap-2 text-[18px]">
                                {!! config('icon.chevronRight') !!}
                                {{ $page->name }}
                            </a>
                        @endforeach
