...
@section('title', 'Business Category Management')
@section('styles')
    <style>
        .dropdown-menu li a {
            display: flex;
            ...
            <div class="d-flex align-items-center justify-content-between flex-wrap">
                <nav>
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">Business Categorys</li>
                                ...
                                <tbody>
                                    @foreach ($categories as $item)
                                        <tr>
                                            <td>
                                                @if ($item->thumbnail)
                                                    <img src="{{ asset($item->thumbnail) }}" class="rounded" alt=""
                                                        width="50">
                                                @else
                                                    <img src="{{ asset('admin/assets/img/400X500.svg') }}" class="rounded"
                                                        alt="" width="50">
                                                @endif
                                                    ...
                                                    <div class="dropdown">
                                                        ...
                                                        <ul class="dropdown-menu">
                                                            <li><a class="dropdown-item"
                                                                    href="{{ route('business-category.edit', $item->id) }}"><i
                                                                        class="bx bxs-edit-alt"></i> Edit</a></li>
                                                            <li>
                                                                <form
                                                                    action="{{ route('business-category.destroy', $item->id) }}"
                                                                    method="POST">
                                                                    @csrf
                                                                    @method('DELETE')
                                                            ...
                                                            <li>
                                                                <a href="{{ route('business.categories.status.update', $item->id) }}"
                ...
                <div class="col-lg-3">
                    <div class="card">
                        @if ($business_category)
                            <div class="card-body">
                                <form class="row g-2"
                                    action="{{ route('business-category.update', $business_category->id) }}" method="POST"
                                    enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                    <div class="col-12 p-2 text-center">
                                        <div>
                                            <img id="profileImage" src="{{ asset($business_category->thumbnail) }}"
                                                alt="image" width="30%">
                                            <div class="pt-2">
                                                <!-- Hidden File Input -->
                                                <input type="file" name="thumbnail" id="profileImageInput"
                                                    style="display: none;">
                                    ...
                                    <div class="col-12 mb-2">
                                        ...
                                        <select name="is_active" id="" class="form-control" required>
                                            <option {{ $business_category->is_active == 1 ? 'selected' : '' }} value="1">
                                                Active</option>
                                            <option {{ $business_category->is_active == 0 ? 'selected' : '' }} value="0">
                                                Inactive</option>
                                        </select>
                                        @error('is_active')
                                            <span class="text-danger">{{ $message }}</span>
                        ...
                        @else
                            <div class="card-body">
                                <form class="row g-2" action="{{ route('business-category.store') }}" method="POST"
                                    enctype="multipart/form-data">
                                    @csrf
                                    <div class="col-12 p-2 text-center">
                                        <div>
